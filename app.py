...
with st.expander("📈 成长曲线", expanded=True):
        st.subheader("📊 成长数据")
        fig = growth_curve()
        if fig:
            st.pyplot(fig)
...
with st.expander("📜 知识图鉴", expanded=False):
...
with st.expander("📋 任务", expanded=False):
        st.subheader("📋 每日任务")
...
with st.expander("🏅 徽章", expanded=False):
        st.subheader("🏅 已解锁徽章")
...
with st.expander("📅 修行周报", expanded=False):
