...
**🧪 实验场**：真实数据回测沙盒——**9 种策略模板**（定投 / 均线择时 / 等权组合 / 风险平价 / 动量轮动 + 经典指标：MACD 金叉 / RSI 超买超卖 / 布林带均值回归 / 双推力突破），选资产、拖参数（默认值即经典配置）、一键出净值曲线 + 夏普 + 最大回撤 + 水下曲线，结果可存档并完成周任务「回测复现」（+30 XP）。指标策略移植自 je-suis-tm/quant-trading（MIT）。
