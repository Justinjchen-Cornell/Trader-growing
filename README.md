...
第 4 章「何时动」4 关（再平衡→止损→止盈→BOSS 锯齿效应）、
第 5 章「体检台」4 关（基准对比→逐年拆解→回撤深度→BOSS 参数敏感性）、
第 6 章「陷阱迷宫」4 关（样本内外→Walk-forward→交叉验证→BOSS 规则负担）、
...
**⚡ 今日一题**：每天 30 秒——用「今天的真实行情」出一道量化题（定投份数/均线方向/相关性/风险平价/止损价……8 种题型按日轮换，答案每天不同）。答对 +3 XP，可下载**分享卡片 PNG** 发微信群。

**🧪 实验场**：真实数据回测沙盒——**15 种策略模板**（每张都配「策略课」知识卡：是什么/怎么调参/何时失效）（定投 / 均线择时 / 等权组合 / 风险平价 / 动量轮动 + 经典指标：MACD 金叉 / RSI 超买超卖 / 布林带均值回归 / 双推力突破），选资产、拖参数（默认值即经典配置）、一键出净值曲线 + 夏普 + 最大回撤 + 水下曲线，结果可存档并完成周任务「回测复现」（+30 XP）。指标策略移植自 je-suis-tm/quant-trading（MIT）。

**🎯 今日操作台**：看天气（四资产看板）→ 定计划（中庸策略 plan）→ 对账（30 秒三问：计划外交易/移动止损/未执行）——纪律核心在 Web 端闭环。数据一键更新（akshare，免费免注册）。
...
- **V2.7**：第 4 章 4 关（再平衡 / 止损 / 止盈 / 锯齿 BOSS），共 16 关 ✅
- **V2.8**：第 5 章 4 关（基准 / 逐年拆解 / 回撤 / 参数敏感性 BOSS），共 20 关 ✅
- **V2.9**：第 6 章 4 关（样本内外 / Walk-forward / 交叉验证 / 规则负担 BOSS），共 24 关 ✅
...
- **V5.5**：⚡ 今日一题 + 分享卡片 / 🧪 实验场回测沙盒 / 🎯 今日操作台（对账闭环）/ 新手 7 天双倍 XP ✅
